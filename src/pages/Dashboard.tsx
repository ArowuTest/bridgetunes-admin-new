...
import JackpotStatus from '../components/dashboard/JackpotStatus'; // Import the new component
...
// Add a new grid container for Jackpot and Recent Draws
const BottomGrid = styled.div`
  display: grid;
  grid-template-columns: 1fr 2fr; // Adjust ratio as needed
  gap: 1.5rem;
  @media (max-width: ${props => props.theme.breakpoints.lg}) { // Adjust breakpoint if needed
    grid-template-columns: 1fr;
  }
`;
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      ...
      try {
        ...
        // Set mock data or show error message if needed
      } finally {
        setIsLoading(false);
      }
    };
  ...
  return (
    <PageLayout title="Dashboard">
      <DashboardContainer>
        ...
        <StatsGrid>
          <StatsCard title="Total Users" value={stats?.totalUsers || 0} icon={<FaUsers />} color="primary" />
          <StatsCard title="Total Revenue" value={stats?.totalRevenue || 0} icon={<FaMoneyBillWave />} color="success" isCurrency />
          <StatsCard title="Total Draws" value={stats?.totalDraws || 0} icon={<FaCalendarAlt />} color="info" />
          <StatsCard title="Total Winners" value={stats?.totalWinners || 0} icon={<FaBullhorn />} color="warning" />
        </StatsGrid>
        ...
        {/* New Bottom Grid for Jackpot and Recent Draws */}
        <BottomGrid>
          {/* Integrate JackpotStatus component */}
          <JackpotStatus />

          {/* Keep Recent Draws Table */}
          <Card>
            <RecentDrawsTable
              title="Recent Draws"
